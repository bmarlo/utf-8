 ...
 * bytes.
 */
std::vector<std::uint32_t> decode(std::string_view u8);
 ...
 * string, so the remainder of the string may still contain invalid utf-8 bytes.
 */
std::optional<std::uint32_t> char_at(std::string_view u8, std::size_t i) noexcept;
 ...
 */
std::optional<std::size_t> char_count(std::string_view u8) noexcept;
 ...
 * On success, the index parameter is updated to one byte past the code point.
 */
std::optional<std::uint32_t> next_char(std::string_view u8, std::size_t& i) noexcept;
...
inline std::vector<std::uint32_t> decode(std::string_view u8)
{
...
inline std::optional<std::uint32_t> char_at(std::string_view u8, std::size_t i) noexcept
{
    std::uint32_t code;
    if (char_at(u8, i, code)) {
        return code;
    }
    return std::nullopt;
}

inline std::optional<std::size_t> char_count(std::string_view u8) noexcept
{
    std::size_t count;
    if (char_count(u8, count)) {
        return count;
    }
    return std::nullopt;
}

inline std::optional<std::uint32_t> next_char(std::string_view u8, std::size_t& i) noexcept
{
    std::uint32_t code;
    if (next_char(u8, i, code)) {
        return code;
    }
    return std::nullopt;
}
